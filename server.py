...
INDEX_HTML = HTML_PATH.read_text(encoding="utf-8") if HTML_PATH.exists() else ""
INDEX_HTML_MTIME = HTML_PATH.stat().st_mtime if HTML_PATH.exists() else 0
...
@app.route("/")
def index():
    global INDEX_HTML, INDEX_HTML_MTIME
    try:
        mtime = HTML_PATH.stat().st_mtime
        if mtime != INDEX_HTML_MTIME:
